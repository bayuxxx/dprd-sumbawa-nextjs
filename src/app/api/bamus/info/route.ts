...
async function getInfoWithAnggota(id: string) {
...
export async function GET(req: NextRequest) {
  ...
  const params = id ? [id] : [];
  const [rows]: any = await db.query(`SELECT * FROM bamus_info ${where} LIMIT 1`, params);
  if (rows.length === 0) return NextResponse.json(null);
  const [anggota] = await db.query('SELECT * FROM anggota_bamus WHERE bamusInfoId = ? ORDER BY `order` ASC', [rows[0].id]);
...
export async function POST(req: NextRequest) {
  ...
  if (isAktif === true || isAktif === 'true') await db.query('UPDATE bamus_info SET isAktif = 0');
  const id = crypto.randomUUID().replace(/-/g, '').substring(0, 25);
  const now = new Date();
  await db.query('INSERT INTO bamus_info (id, masaJabatan, deskripsi, isAktif, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)',
    [id, masaJabatan, deskripsi || null, isAktif === true || isAktif === 'true' ? 1 : 0, now, now]);
  const item = await getInfoWithAnggota(id);
