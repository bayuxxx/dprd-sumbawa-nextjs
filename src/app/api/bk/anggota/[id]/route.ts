...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (file && file.size > 0) {
      ...
      imageUrl = await processFileUpload(file, 'bk');
    }
    ...
    const now = new Date();
    await db.query('UPDATE anggota_bk SET name = ?, jabatan = ?, faction = ?, imageUrl = ?, `order` = ?, bkInfoId = ?, updatedAt = ? WHERE id = ?',
      [name || existing.name, jabatan || existing.jabatan, faction !== undefined ? faction : existing.faction, imageUrl, order !== undefined ? parseInt(order) : existing.order, bkInfoId !== undefined ? (bkInfoId || null) : existing.bkInfoId, now, id]);
    const [updated]: any = await db.query('SELECT * FROM anggota_bk WHERE id = ?', [id]);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.query('DELETE FROM anggota_bk WHERE id = ?', [id]);
