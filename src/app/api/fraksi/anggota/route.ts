...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (file && file.size > 0) imageUrl = await processFileUpload(file, 'fraksi-anggota');
    ...
    await db.query(
      ...
      [id, name, jabatan, faction || null, imageUrl, order ? parseInt(order) : 0, fraksiInfoId || null, now, now]
    );
