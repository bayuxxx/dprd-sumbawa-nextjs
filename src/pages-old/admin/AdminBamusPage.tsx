...
const AdminBamusPage: React.FC = () => {
    const [bamusInfoList, setBamusInfoList] = useState<BamusInfo[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');

    // Info form
    const [showInfoForm, setShowInfoForm] = useState(false);
    const [editingInfo, setEditingInfo] = useState<BamusInfo | null>(null);
    ...
    const [editingAnggota, setEditingAnggota] = useState<AnggotaBamus | null>(null);
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            const data = await fetchAllBamusInfo();
            setBamusInfoList(data);
            if (data.length > 0 && !expandedInfo) {
                setExpandedInfo(data[0].id);
            }
        } catch {
            setError('Gagal memuat data bamus');
        }
        setLoading(false);
    }, [token]);
    ...
    const openEditInfo = (info: BamusInfo) => {
        setEditingInfo(info);
        setInfoForm({
    ...
    const handleSaveInfo = async () => {
        ...
        try {
            if (editingInfo) {
                await updateBamusInfoApi(editingInfo.id, infoForm, token);
            } else {
                await createBamusInfoApi(infoForm, token);
            }
    ...
    // ── Anggota CRUD ──
    const openCreateAnggota = (bamusInfoId: string) => {
        setEditingAnggota(null);
        setAnggotaForm({ ...emptyAnggotaForm, bamusInfoId });
        setImageFile(null);
        setImagePreview('');
        setShowAnggotaForm(true);
    };

    const openEditAnggota = (anggota: AnggotaBamus) => {
        ...
        setAnggotaForm({
            ...
            bamusInfoId: anggota.bamusInfoId || '',
        });
        setImageFile(null);
    ...
    const handleSaveAnggota = async () => {
        ...
        try {
            ...
            fd.append('bamusInfoId', anggotaForm.bamusInfoId);
            if (imageFile) fd.append('image', imageFile);

            if (editingAnggota) {
                await updateAnggotaBamus(editingAnggota.id, fd, token);
            } else {
                await createAnggotaBamus(fd, token);
            }
    ...
    const handleDelete = async () => {
        ...
        try {
            if (deleteTarget.type === 'info') {
                await deleteBamusInfoApi(deleteTarget.id, token);
            } else {
                await deleteAnggotaBamus(deleteTarget.id, token);
            }
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-2xl font-black text-gray-900">Badan Musyawarah</h1>
                        <p className="text-gray-500 text-sm mt-1">Kelola data anggota Bamus per masa jabatan</p>
                    </div>
                    <button
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-6 border-b border-gray-100">
                            <h2 className="text-lg font-black text-gray-900">
                                {editingInfo ? 'Edit Masa Jabatan Bamus' : 'Tambah Masa Jabatan Bamus'}
                            </h2>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Deskripsi tentang Badan Musyawarah..."
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAnggotaForm(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
                            <h2 className="text-lg font-black text-gray-900">
                                {editingAnggota ? 'Edit Anggota Bamus' : 'Tambah Anggota Bamus'}
                            </h2>
...
const AnggotaSection: React.FC<{
    title: string;
    items: AnggotaBamus[];
    onEdit: (a: AnggotaBamus) => void;
    onDelete: (a: AnggotaBamus) => void;
