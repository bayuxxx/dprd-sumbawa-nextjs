...
import { fetchBanggarInfo } from '../../services/api';
...
const BanggarContent: React.FC<{ id?: string }> = ({ id }) => {
    ...
    const anggotaList = banggarInfo?.anggota || [];
    ...
    const masaJabatan = banggarInfo?.masaJabatan || '2024 – 2029';
    const deskripsi = banggarInfo?.deskripsi ||
        'Badan Anggaran (Banggar) merupakan alat kelengkapan DPRD yang dibentuk untuk memberikan saran dan pendapat terkait berbagai kebijakan anggaran, membahas rancangan KUA-PPAS, RAPBD, serta melakukan evaluasi pelaksanaan APBD sesuai dengan arah pembangunan daerah.';
    ...
    return (
        <div className="w-full flex flex-col items-center animate-fade-in">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-gray-900 mb-6 tracking-tight text-center">
                Badan Anggaran <br className="hidden xl:block" /> Masa Jabatan {masaJabatan}
            </h1>
            ...
            {!hasData ? (
                <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
                    ...
                    <p className="text-gray-400 font-medium">Belum ada data anggota Badan Anggaran.</p>
